...
public class ControladorJuego : ControladorMaestro {
    ...
    void Start() {
        ...
        ShowCoins();
        SearchRecords();
        initialDropZone.SetActive(false); //Desactivamos esta zona para que el jugador no vea el mensaje sin haber empezado
    }
    ...
    override public void GameStarts() {
        ...
        CheckPowerUpPrices();
        initialDropZone.SetActive(true); //Reactivamos la zona inicial de drop para que el jugador la vea.
    }
    ...
    override public void SkipQuestion() {
        if (PlayerPrefs.GetInt("Coins") > skipQuestionPrice) {
            ...
            skipQuestionPrice = skipQuestionPrice + 3;
            //skipQuestionPrice = (skipQuestionPrice * 3) / 2; //Aumentamos el precio de pasar pregunta //TODO: Recuperar el precio de antes
            nextQuestionPriceText.text = "" + skipQuestionPrice;
            ShowCoins();
            NewQuestion();
        }
    ...
    override public void PowerUpSetFirstChip() {
        if (!powerupFirstChipSet) //Si ya ha utilizado el power up no se le permite volver a lanzarlo (no tendría sentido)
            if (PlayerPrefs.GetInt("Coins") > firstChipPrice) {
                ...
                firstChipPrice = firstChipPrice + 2;
                setFirstPriceText.text = "" + firstChipPrice;
    ...
    override public void PowerUpDeleteChip() {
        ...
        { //Si ya se ha eliminado todas las fichas.
            if (PlayerPrefs.GetInt("Coins") > deleteChipPrice)
            {
                ...
                deleteChipPrice = deleteChipPrice + 2;
                deleteChipPriceText.text = "" + deleteChipPrice;
                ShowCoins();
                CheckPowerUpPrices();
            }
    ...
    void CheckPowerUpPrices() { //Este metodo se encarga de comprobar los precios de cada powerup
        print(listNoValidChips.Count);
        print(counterDeleteChips);
        ...
        if (deleteChipPrice >= PlayerPrefs.GetInt("Coins") || counterDeleteChips >= listNoValidChips.Count) {
            print("ENTRA AQUI");
    ...
    override public void ResetGame() {
        ...
        SetPrices();
        initialDropZone.SetActive(false);
    }
